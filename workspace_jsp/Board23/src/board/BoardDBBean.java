...
public class BoardDBBean {
	...
	public void insertBoard(BoardDataBean board) {
		...
		String sql3 = "insert into board(writer,email,pwd,subject,content,ref,re_step,re_level) values(?,?,?,?,?,?,?,?)";
		...
		try {
			...
			pstmt.setString(3, board.getPwd());
	...
	public int updateBoard(BoardDataBean board) {
		String sql = "update board set writer=?, email=?, subject=?, content=? where num=? and pwd=?";
		int count = 0;
		...
		try {
			...
			pstmt.setString(6, board.getPwd());
			...
			e.printStackTrace();
			System.out.println("# error : updqteBoard() 메소드에서 예외처리");
