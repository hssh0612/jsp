...
<head>
<meta charset="UTF-8">
<title>updateTestPro</title>
</head>
<body>
	<h2>member 데이터 수정 처리 페이지</h2>
	...
	<%
		...
		// SQL
		String sql1 = "select id, pwd from member where id = ?"; // 아이디, 패스워드 검증
		...
		try {
			...
			// 아이디가 있을 때는 true -> 업데이트 실행
			// 아이디가 없을 때는 false -> 처리 문구
			if(rs.next()) { // 아이디가 있을 때
				String rId = rs.getString("id");
				String rPwd = rs.getString("pwd");
				if(id.equals(rId) && pwd.equals(rPwd)) { // 아이디와 패스워드가 일치한다면
					// 수정 처리
